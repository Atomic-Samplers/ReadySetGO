...
class ReadySetGO:
    def __init__(
        ...
        general_settings_dict={
            "close_contact_cutoff": 0.5,  # not sure where to put this setting, placing in general for now
            "iterations": 1000,
            "verbose": 1,
            "local_run": True,
        },
    ...
    def main(self, live_tracking=True):
        ...
        # create all objects
        initialisation_object = ModuleManager(
            "initialization",
            self.initialization_type,
            self.initialization_settings_dict,
        ).get()
        database_object = ModuleManager(
            "database", self.database_type, self.database_settings_dict
        ).get()
        global_descriptor_object = ModuleManager(
            "structure_clustering.global_descriptors",
            f"{self.global_descriptor_type}_descriptor",
            self.global_descriptor_settings_dict,
        ).get()
        clustering_object = ModuleManager(
            "structure_clustering.clustering_algorithms",
            f"{self.clustering_algorithm_type}_clustering_algorithm",
            self.clustering_algorithm_settings_dict,
        ).get()
        go_object = ModuleManager(
            "global_optimization",
            self.global_optimization_type,
            self.global_optimization_settings_dict,
        ).get()
        lo_object = ModuleManager(
            "local_optimization",
            self.local_optimization_type,
            self.local_optimization_settings_dict,
        ).get()
        ...
        database_object.set_attribute(
            "iterations", self.general_settings_dict["iterations"]
        )
        clustering_object.set_attribute("base_atoms", base_atoms_object)
        clustering_object.set_attribute(
            "iterations", self.general_settings_dict["iterations"]
        )
        clustering_object.set_attribute(
            ...
            while iteration <= self.general_settings_dict["iterations"]:
                ...
                while similarity:
                    ...
                    while close_contacts:
                        ...
                        close_contacts = detect_close_contacts(
                            go_suggested_atoms,
                            self.general_settings_dict["close_contact_cutoff"],
                        )
                    ...
                    if (
                        len(atoms_list) > 1
                        and self.clustering_algorithm_type != "dummy"
                    ):
                        global_descriptor_object.set_attribute(
                            "structure", go_suggested_atoms
                        )
                        clustering_object.set_attribute(
                            "global_descriptor_object", global_descriptor_object
                        )
                ...
                if self.general_settings_dict["verbose"] > 0:
                    print(
                        f"Iteration {iteration}: Global optimization suggested atoms after {cc_count} close contact checks and {pre_lo_sim_count} similarity checks.",
                        flush=True,
                    )
                ...
                if lo_atoms.info["relaxed"]:
                    ...
                    database_object.set_attribute(
                        "go_suggested_atoms", go_suggested_atoms
                    )
                    ...
                    row = f"| {lo_atoms.info['id']:<12} | {lo_atoms.info['go_method']:<24} | {lo_atoms.get_potential_energy():>12.6f} | {time_to_opt:>17} | {lo_steps:>9} |"
